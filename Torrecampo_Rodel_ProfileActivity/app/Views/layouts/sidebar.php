        ...
        <nav class="mt-2">
            <ul class="nav sidebar-menu flex-column" data-lte-toggle="treeview" role="navigation" aria-label="Main navigation" data-accordion="false" id="navigation">
                ...
                <?php foreach ($MenuCategory as $mCategory) : ?>
                    <?php if ($mCategory['menu_category'] != 'Common Page' && !in_array($mCategory['menu_category'], $renderedHeaders)) : ?>
                        <li class="nav-header"><?= $mCategory['menu_category'] ?></li>
                        <?php $renderedHeaders[] = $mCategory['menu_category']; ?>
                    <?php endif; ?>
                    <?php
                    // Ensure menuCategoryID and user role are valid before fetching menus
                    if (!isset($mCategory['menuCategoryID'], $user['role']) || $mCategory['menuCategoryID'] === null || $user['role'] === null) {
                        continue; // Skip this category if required data is missing
                    }

                    $Menu = getMenu($mCategory['menuCategoryID'], $user['role']);
                    foreach ($Menu as $menu) :
                        if (in_array($menu['title'], $renderedMenus) || $menu['title'] == 'Dashboard') {
                            continue;
                        }
                        $renderedMenus[] = $menu['title'];
